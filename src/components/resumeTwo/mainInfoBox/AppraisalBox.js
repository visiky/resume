        ...
        <div className = "appraisal-box">
            <div className="section-list-item">
            {
                appraisals.map((appraisal,index) => {
                    return <Tooltip placement="right" title={"右键删除"}  key={appraisal+'-'+index}><p style={{ textIndent: '30px'}} className="section-list-item__content" onContextMenu = { (e) => {handleContextMenu( e,index )} }>{ appraisal }</p></Tooltip>
                })
            }
            </div>
        </div>
