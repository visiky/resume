...
class ExperienceItem extends Component {
    ...
    render(){
        const { styleColor, index, experienceType, imgUrl, experienceDescription } = this.props || {};
        return (
            experienceType ?
            <Tooltip placement="right" title={"右键删除,拖拽移动"}>
                <div className="section-list-item" onContextMenu={(e) => this.props.handleContextMenu(index,e)} title="右键删除">
                {
                    ((url) => {
                    if (url) 
                        return (
                            <img src={url} alt='项目介绍' width="30px" height="30px" style={{ marginRight: '8px' }}/>
                        )
                    })(imgUrl)
                }
                <div>
...
class ExperienceBox extends Component {
    ...
    moveItem(fromIndex,toIndex){
        let nextExperiences = this.props.experiences;
        [ nextExperiences[fromIndex], nextExperiences[toIndex] ] = [ nextExperiences[toIndex], nextExperiences[fromIndex] ];
            ...
            <div className="experience-box">
                {    
                    experiences.map((experience,index) => {
                        return <DragItem key={'experience-' + index } { ...experience } index = {index} styleColor = {styleColor} handleContextMenu = { this.handleContextMenu } moveItem = {this.moveItem}/>
                    })
                }
            </div>
        )
